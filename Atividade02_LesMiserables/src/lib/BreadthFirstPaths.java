...
public class BreadthFirstPaths {
    ...
    private int[] spCount; // Contagem de vezes em que um menor caminho de s-v passou por cada vértice
    private double[] betweennessCentrality; // Centralidade de intermediação de cada vértice
    ...
    public int greatestDist() {
        int max = this.distTo[0];

        for (int i = 1; i < this.distTo.length; i++) {
            if (this.distTo[i] > max) {
                max = this.distTo[i];
            }
        }
    ...
    public int distToCount() {
        // Conta o número de distâncias mais curtas entre s e v.
